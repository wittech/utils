 ...
 * @param number
 */
export function getNumberResult(number: string | number): NumberResult | undefined {
 ...
 * @param arrays
 */
export function centerArray<T>(baseArr: T[], ...arrays: T[][]) {
  return Object.assign(
    baseArr,
    arrays.reduce((p, c) => ({ ...p, ...c.splice(0, c.length) }), {})
  );
}
 ...
 */
export function unshiftZero(arr: number[], num: number = 1) {
  if (num <= 0) return arr;
  if (isNil(num)) {
    num = 1;
  }
 ...
 * @param lang 转换语言
 */
export function encodeInteger(integer: string, options: Options, lang: Lang): string {
  ...
  if (_length <= 4) {
    for (let i = 0, n = _length; n--; ) {
      ...
      _integer += _tenMin && _length == 2 && i == 0 && _num == 1 ? '' : digits.charAt(_num);
    ...
    _integer =
      encodeInteger(_maxLeft, options, lang) +
      unitChars.charAt(3 + d) +
